...
export async function POST(
  ...
  if (!parsed.success)
    return NextResponse.json({ error: { message: "Enter a rejection reason." } }, { status: 400 });
  const { response, body } = await authenticatedJson(`/admin/billing/payments/${id}/reject`, {
    method: "POST",
