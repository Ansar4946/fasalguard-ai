...
import type { FarmSummary, FieldSummary } from "@/components/fields/types";
...
export default async function FieldsPage({ params }: { params: Promise<{ farmId: string }> }) {
  ...
  const [farmResult, fieldsResult] = await Promise.all([
    authenticatedJson<FarmSummary>(`/farms/${farmId}`),
    authenticatedJson<FieldSummary[]>(`/farms/${farmId}/fields`),
  ]);
  const { response, body } = farmResult;
  ...
  if (!fieldsResult.response.ok)
    return (
      <div className="mx-auto max-w-[1180px] p-4 md:p-6 xl:p-8">
        <p role="alert" className="rounded-xl border border-danger/20 bg-danger/5 px-4 py-3 text-xs font-semibold text-danger">
          Could not load this farm&apos;s fields right now. Try refreshing the page.
        </p>
      </div>
    );
  ...
  return (
    <FieldsManagement
      farm={{ id: body.id, name: body.name }}
      initialFields={fieldsResult.body}
    />
  );
}
