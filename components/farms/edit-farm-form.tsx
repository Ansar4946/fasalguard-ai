...
import { fromGeoJsonPolygon, toGeoJsonPolygon, type GeoJsonPolygon } from "@/lib/geo/polygon";

const inputClass =
  "h-10 w-full rounded-lg border border-[#ccd6cf] bg-[#f7f8fc] px-3 text-[11px] outline-none focus:border-brand";

export interface EditFarmFormData {
  id: string;
  name: string;
...
export function EditFarmForm({ farm }: { farm: EditFarmFormData }) {
  const router = useRouter();
  const [name, setName] = useState(farm.name);
  const [province, setProvince] = useState(farm.province ?? "");
  const [district, setDistrict] = useState(farm.district ?? "");
  const [tehsil, setTehsil] = useState(farm.tehsil ?? "");
  const [soilType, setSoilType] = useState(farm.soilType ?? "");
  const [irrigationType, setIrrigationType] = useState(farm.irrigationType ?? "");
  const [waterSource, setWaterSource] = useState(farm.waterSource ?? "");
  const [points, setPoints] = useState<[number, number][]>(() => fromGeoJsonPolygon(farm.boundary));
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (!response.ok) {
        setError(body.error?.message ?? "Could not save the farm. Try again.");
        return;
      }
